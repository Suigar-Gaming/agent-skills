...
2. Use the action-specific `client.suigar.tx.pvpCoinflip` builder.
3. Pass `gameId` to `joinGame` and `cancelGame`.
...
7. Decode any accompanying standard `BetResultEvent.game_details` with [event and numeric parsing](../../installation/references/events.md).
...
When a flow also decodes `BetResultEvent`, use [event and numeric parsing](../../installation/references/events.md) for the canonical `parseSuigarEvent`, `parseGameEvent`, and `parseGameDetails` pattern.
...
2. Wire the flow to the matching `pvpCoinflip.createGame`, `joinGame`, or `cancelGame` builder.
...
7. Parse `BetResultEvent.game_details` with [event and numeric parsing](../../installation/references/events.md) when displaying bet result details.
